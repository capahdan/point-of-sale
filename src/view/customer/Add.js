...
export default function CustomerAdd() {
  ...
  const HandleSubmit2 = async e=>{
    e.preventDefault();    
    try{
      await axios.post('http://localhost:5001/v1/customer',
        {
          name,
          email,
          phone_number,
        ...
        navigate('/customer', {state:"ditambahkan"})
      } catch(error){
        console.log(error)
      }
}
...
const HandleSubmitEdit = async e=>{
  e.preventDefault();    
  try{
    await axios.put(`http://localhost:5001/v1/customer/${state.id}`,
      {
        name,
        email,
        phone_number,
      ...
      navigate('/customer', {state:"diedit"})
    } catch(error){
      console.log(error)
    }
}
          ...
          <Paper sx={{width:'100%', mb:2,height:500, boxShadow:'20'}} >
            ...
            > 
               <Typography
                ...
                component="div"
                >
               {state === null ? 'Add Customer' : 'Edit Customer'} 
               </Typography>
            </Toolbar>
            <form onSubmit={state===null ? HandleSubmit2: HandleSubmitEdit}>
              <Container>
                ...
                <Button color="error" sx={{marginLeft:'0.5rem'}} variant="contained"><Link sx={{textDecoration:'none', color:"inherit"}} href="/customer" underline="always">Cancel</Link></Button>
                </div>
              </Container>
            </form>
          </Paper>
